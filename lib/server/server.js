...
pro.globalHandle = function(msg, session, cb) {
  ...
  //{ id: 1,type: 0,compressRoute: 0,route: 'connector.entryHandler.entry',body: 'hello pomelo',compressGzip: 0 }
...
var handleError = function(isGlobal, server, err, msg, session, resp, opts, cb) {
  var handler;
  //这两个函数通过app.set('errorHandler', ...);
  //https://github.com/NetEase/pomelo/issues/204
  if(isGlobal) {
